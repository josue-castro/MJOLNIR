export default {
  ...
  async addEvent({ commit, dispatch }, eventJSON) {
    try {
      const response = await this.$axios.post(
        "events/team/" + eventJSON.team_id + "/",
        eventJSON
      );
      commit("ADD_EVENT", response.data.Event);
      dispatch(
        "notifications/setSnackbar",
        { text: "Se añadió un nuevo evento exitosamente", color: "success" },
        { root: true }
      );
    } catch (error) {
      if (!!error.response.data) {
  ...
  async editEvent({ commit, dispatch }, eventJSON) {
    try {
      ...
      dispatch(
        "notifications/setSnackbar",
        {
          text: `El evento con identificador:${eventJSON.event_id} ha sido editado.`,
          color: "success"
        },
        { root: true }
      );
  ...
  async removeEvent({ commit, dispatch }, eventID) {
    try {
      ...
      dispatch(
        "notifications/setSnackbar",
        {
          text: `El evento con identificador:${eventID} ha sido removido.`,
          color: "success"
        },
        { root: true }
      );
  ...
  async startPBPSequence({ commit, dispatch }, eventJSON) {
    try {
      ...
      const response = await this.$axios.post("pbp/" + eventJSON.sport_name, {
      ...
      this.$router.push(
        "/jugadas-" + eventJSON.sport_name + "/" + eventJSON.event_id
      );
    } catch (error) {
      if (!!error.response.data) {
        dispatch(
